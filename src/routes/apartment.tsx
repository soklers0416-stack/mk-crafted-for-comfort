...
export const Route = createFileRoute("/apartment")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Подберите мебель для всей квартиры в одном месте." },
    ],
  }),
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="mx-auto max-w-7xl px-4 py-12 md:px-8 md:py-16">
        ...
        <h1 className="mt-2 font-display text-4xl font-bold tracking-tight md:text-5xl">Квартира под ключ</h1>
        ...
        <div className="mt-10 grid grid-cols-3 gap-2 md:grid-cols-6">
          {steps.map((s, i) => (
            <button key={s.slug} onClick={() => setStep(i)}
              className={`rounded-2xl border px-3 py-3 text-left text-xs transition ${
                i === step ? "border-primary bg-primary/5" : picked[s.slug] ? "border-primary/40 bg-primary/5" : "border-border bg-card"
              }`}>
              <div className="flex items-center gap-1.5 text-[11px] font-medium text-primary">
                Шаг {i + 1}{picked[s.slug] && <Check className="h-3 w-3" />}
              </div>
        ...
        <div className="mt-10 grid gap-8 lg:grid-cols-[1fr_320px]">
          <div>
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => setPicked((p) => ({ ...p, [current.slug]: null }))}
                className="text-sm text-muted-foreground hover:text-primary">Пропустить</button>
            </div>
            {list.length === 0 ? (
              ...
              <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2">
                {list.map((p) => {
                  ...
                  return (
                    <button key={p.id} onClick={() => setPicked((s) => ({ ...s, [current.slug]: p.id }))}
                      ...
                      <div className="aspect-[5/4] overflow-hidden bg-surface-muted">
                        {p.photo1 && <img src={p.photo1} alt={p.title} loading="lazy" className="h-full w-full object-contain p-3 transition group-hover:scale-105" />}
                      </div>
          ...
          <aside className="rounded-3xl border border-border/60 bg-card p-6">
            ...
            <button onClick={finish}
              ...
              <ShoppingBag className="h-4 w-4" />Отправить в корзину
            </button>
