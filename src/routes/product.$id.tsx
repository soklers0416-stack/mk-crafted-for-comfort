        ...
        <div className="mt-6 grid gap-10 lg:grid-cols-2">
          ...
          <div>
            ...
            {hasSizes && (() => {
              ...
              for (const r of rows) {
                const s = (r.sleeping ?? "").trim();
                if (r.size && s && !sleepingBySize.has(r.size)) sleepingBySize.set(r.size, s);
              }
              const sleepingOptions = Array.from(new Set(Array.from(sleepingBySize.values())));
              if (sleepingOptions.length === 0) return null;
              const currentSleeping = sleepingBySize.get(effectiveSize) ?? "";
              return (
                <div className="mt-5">
                  <div className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Спальное место</div>
                  <div className="mt-2 flex flex-wrap items-center gap-2">
                    {sleepingOptions.map((sl) => {
                      const active = sl === currentSleeping;
                      return (
                        <button
                          key={sl}
                          onClick={() => {
                            const match = rows.find((r) => (r.sleeping ?? "").trim() === sl && r.size);
                            if (match?.size) setSelSize(match.size);
                          }}
                          className={`rounded-full border px-4 py-2 text-sm font-medium transition ${
